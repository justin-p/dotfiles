...
- `better-commits` wrapper in `zsh/.zsh_alias.d/30-git.zsh`: bare `git commit` or bare `git add` (no arguments) invokes `better-commits` when installed; otherwise prints a prefer-tool hint with `npm install -g better-commits` and falls through to git. `git add <file>`, `git commit -m`, etc. always pass through to git. `dotfiles-sync-fix` uses `command git commit -m …` so it does not invoke better-commits.
